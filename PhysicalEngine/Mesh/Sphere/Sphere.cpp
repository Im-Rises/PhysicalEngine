...
void Sphere::generatePointsNormales( double radius, int rings, int sectors) {
	...
	//float s, t;									 // vertex texture Coordonnées
	double pi = M_PI;
	...
	for (int i = 0; i <= rings; ++i) {
		...
		for (int j = 0; j <= rings; ++j) {
			...
			//coordonnées du vertex pour les textures  (s, t) entre [0, 1]
			/*
			s = (float)j / sectors;
			t = (float)i / rings;
...
void Sphere::generateTriangles( double radius, int rings, int sectors) {
	int k1, k2;
	for (int i = 0; i < rings; ++i) {
		k1 = i * (sectors + 1); // début du ring actuel
		k2 = k1 + sectors + 1;	// début du prochain ring
